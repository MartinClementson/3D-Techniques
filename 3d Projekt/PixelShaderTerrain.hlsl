...
//Hardcoded fog, Do be able to change this in UI, add this to a separate constant buffer
float gFogStart = 10.0f; //The distance where the fog will start from the camera.
float gFogRange = 100.0f; //the range of the fog. the longer the range, the more levels of fog shading it'll be
float4 gFogColor = { 0.5 ,0.5f ,0.5 ,1.0 };
...
float3 distanceVec = (input.camPos - input.wPos.xyz);

float distance = length(distanceVec);

float3 v = normalize(distanceVec);
...
//blend the fog and the color;
col = lerp(col, gFogColor, fogLerp);
